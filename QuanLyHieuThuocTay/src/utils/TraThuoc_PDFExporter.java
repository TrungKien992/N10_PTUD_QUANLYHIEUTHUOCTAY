...
public class TraThuoc_PDFExporter {

    public static final String FONT_PATH = "fonts/times.ttf"; // Đảm bảo file này tồn tại
    private static final double TAX_RATE = 0.1; // 10% tax rate, consistent with DoiTra_Controller
    private static DecimalFormat df = new DecimalFormat("#,##0 VND");
    private static DateTimeFormatter dtf = DateTimeFormatter.ofPattern("dd/MM/yyyy");

    public static void exportPhieuTraThuocToPdf(PhieuDoiTra pdt, ArrayList<CTPhieuDoiTra> dsCTPDT, String filePath) {
        ...
        if (pdt == null || dsCTPDT == null || dsCTPDT.isEmpty()) {
            JOptionPane.showMessageDialog(null, "Không có dữ liệu phiếu trả thuốc để xuất PDF.", "Lỗi Dữ Liệu", JOptionPane.ERROR_MESSAGE);
            return;
        }

        if (pdt.getHoaDon() == null || pdt.getHoaDon().getMaHD() == null) {
            JOptionPane.showMessageDialog(null, "Phiếu trả thuốc không có hóa đơn liên kết hợp lệ.", "Lỗi Dữ Liệu", JOptionPane.ERROR_MESSAGE);
            return;
        }
        ...
        // Declare soLuongCuMap outside the try block
        ...
        try (PdfWriter writer = new PdfWriter(filePath);
            ...
            document.add(new Paragraph("PHIẾU TRẢ THUỐC")
                    .setFont(vietnameseFont).setFontSize(20).setBold()
                    .setTextAlignment(TextAlignment.CENTER)
                    .setMarginTop(20).setMarginBottom(10));

            document.add(new Paragraph("Mã Phiếu Trả: " + pdt.getMaPDT())
            ...
            document.add(new Paragraph("Khách Hàng: " + (pdt.getHoaDon() != null && pdt.getHoaDon().getKhachHang() != null ? pdt.getHoaDon().getKhachHang().getTenKH() : "Khách vãng lai"))
                    .setFont(vietnameseFont).setFontSize(12));
            document.add(new Paragraph("Lý Do Trả: " + pdt.getLyDo())
                    .setFont(vietnameseFont).setFontSize(12)
                    .setMarginBottom(15));

            Table table = new Table(UnitValue.createPercentArray(new float[]{1, 4, 2, 2, 2, 2, 3, 3}));
            ...
            table.addHeaderCell(createCell("Số Lượng Trả", true, vietnameseFont));
            table.addHeaderCell(createCell("Tiền Hoàn", true, vietnameseFont));
            ...
            for (CTPhieuDoiTra ct : dsCTPDT) {
                ...
                double giaBan = ct.getThuoc().getGiaBan() != 0 ? ct.getThuoc().getGiaBan() : 0.0; // Fallback if giaBan is null
                int soLuongTra = ct.getSoLuong();
                double tienHoan = giaBan * soLuongTra * (1 + TAX_RATE);
                Integer soLuongCu = soLuongCuMap.getOrDefault(ct.getThuoc().getMaThuoc(), 0); // Now accessible
                ...
                table.addCell(createCell(String.valueOf(soLuongTra), false, vietnameseFont));
                table.addCell(createCell(df.format(tienHoan), false, vietnameseFont));
            ...
            double thueValue = tongTienHoan * TAX_RATE / (1 + TAX_RATE);
            summaryTable.addCell(createSummaryCell(String.format("Thuế (%.0f%% VAT):", TAX_RATE * 100), TextAlignment.RIGHT, vietnameseFont));
            ...
            if (pdt.getHoaDon() != null && pdt.getHoaDon().getKhuyenMai() != null) {
                double giaTriKM = pdt.getHoaDon().getKhuyenMai().getGiaTri();
                khuyenMaiGiam = tongTienHoan * giaTriKM / 100.0 / (1 + TAX_RATE);
                kmText = String.format("-%.0f%% (%s)", giaTriKM, df.format(khuyenMaiGiam));
            }
            ...
            summaryTable.addCell(createSummaryCell("Tổng Tiền Hoàn:", TextAlignment.RIGHT, true, vietnameseFont));
            summaryTable.addCell(createSummaryCell(df.format(tongTienHoan), TextAlignment.LEFT, true, vietnameseFont));
            ...
            JOptionPane.showMessageDialog(null, "Lỗi khi tạo file PDF cho phiếu trả " + pdt.getMaPDT() + ": " + e.getMessage(), "Lỗi Xuất PDF", JOptionPane.ERROR_MESSAGE);
        } catch (Exception e) {
    ...
    private static Cell createSummaryCell(String content, TextAlignment alignment, PdfFont font) {
        return createSummaryCell(content, alignment, false, font);
    }
